...
namespace Nix {
	...
	DrawDataMemoryHeap::Allocation DrawDataMemoryHeap::allocateTriangles(uint32_t _triNum)
	{
		uint32_t loc = _triNum < 16 ? 0 : 1;
		std::vector< Heap >& heaps = m_heapCollection[loc];
		//
		size_t offset;
		uint16_t allocateId;
		for (auto& heap : heaps) {
			bool allocRst = heap.allocator.allocate(_triNum * sizeof(UIVertex) * 3, offset, allocateId);
			if (allocRst) {
				DrawDataMemoryHeap::Allocation allocation;
				allocation.allocateId = allocateId;
				allocation.allocator = &heap.allocator;
				allocation.size = _triNum * sizeof(UIVertex) * 3;
				allocation.ptr = heap.memory + offset;
				return allocation;
			}
		}
