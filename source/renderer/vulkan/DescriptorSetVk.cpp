...
namespace nix {
	...
	VkDescriptorSet DescriptorSetPool::allocateDescriptorSet(MaterialVk* _material, uint32_t _index, uint32_t& _poolIndex) {
		VkDescriptorSetLayout layout = _material->getDescriptorSetLayout(_index).m_descriptorSetLayout;
		...
		for (size_t poolIndex = 0; poolIndex < m_descriptorChunks.size(); ++poolIndex)
		{
			...
			if (pool.available()) {
				set = pool.allocate( m_context->getDevice(), layout);
		...
		if (VK_NULL_HANDLE == set)
		{
			...
			pool.initialize( m_context->getDevice() );
			set = pool.allocate(m_context->getDevice(), layout);
...
// 	}
	...
	void DescriptorSetPoolChunk::initialize(VkDevice _device)
	{
		VkDescriptorPoolSize descriptorPools[] = {
			{}, {}
		};
		descriptorPools[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC;
		descriptorPools[0].descriptorCount = ChunkUniformBlockCount;
		descriptorPools[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		descriptorPools[1].descriptorCount = ChunkSamplerCount;
		//
		VkDescriptorPoolCreateInfo descriptorPoolInfo = {}; {
			descriptorPoolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
			descriptorPoolInfo.pNext = nullptr;
			descriptorPoolInfo.poolSizeCount = sizeof(descriptorPools) / sizeof(VkDescriptorPoolSize);
			descriptorPoolInfo.pPoolSizes = descriptorPools;
	...
	void DescriptorSetPool::initialize(ContextVk* _context) {
		m_context = _context;
	}
}